...
def auto_regist(i,shared_user,n_user,shared_log,win_h,win_w,auto_submit=False):
	...
	try:
		...
		try:
			...
			# IMPORTANT: UNCOMMENT THIS LINE FOR AUTO S	UBMIT
			if (auto_submit):
		...
		if (user['status'] ==""):
			user['status'] = "FAIL at first"
			shared_log.append(f"{user['arc']} {user['name']} FAIL: {str(e)}\n")



class App:
	def __init__(self, root,file_csv="users.csv"):
		...
		self.var_auto_submit = tk.BooleanVar()
		self.check_auto_submit = tk.Checkbutton(root,variable=self.var_auto_submit,text="Auto Submit",font=("Arial", 12), fg='red', justify=tk.LEFT)
	...
	def run_processes(self):
		...
		for i in range(len(self.shared_dict)):
			p = Process(target=auto_regist, args=(i,self.shared_dict, num_users,self.shared_log,self.win_h, self.win_w,self.var_auto_submit.get()))
			p.start()
			processes.append(p)
		for p in processes:
			p.join()
